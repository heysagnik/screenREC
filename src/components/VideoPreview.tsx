  ...
  (
    ...
    const renderCameraOverlay = useMemo(() => {
      if (selectedLayout === 'circle') {
        return (
          <div
            className={`absolute w-56 h-56 rounded-full overflow-hidden bg-gray-900 border-4 border-white transition-all ${cameraPositionClasses} ${
      ...
      return (
        <div
          className={`absolute w-56 h-40 rounded-xl overflow-hidden bg-gray-900 border-4 border-white transition-all ${cameraPositionClasses} ${
            isDragging ? 'cursor-grabbing' : 'cursor-grab'
          }`}
          onMouseDown={onCameraDragStart}
        >
    ...
    return (
      ...
      >
        ...
        {isRecording && (
          <div className="absolute top-4 right-4 flex items-center gap-2 px-4 py-2 bg-red-600 rounded-lg text-white text-sm font-semibold z-20">
